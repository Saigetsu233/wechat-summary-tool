...
- **单页图片日报**：AI 挑选重点，渲染为带彩色话题卡和漫画插画的 `1440×2400` 长版信息图
- **AI 话题配图**：NVIDIA 图片模型一次生成整张插画板，再裁成最多 10 张话题配图
...
1. 选择 PNG 保存位置。
2. AI 先将聊天压缩为 5～10 个核心话题、今日 MVP、5～10 个趣味成就和 1 条金句。
3. 启用“AI 话题插画”时，NVIDIA 图片模型一次生成无文字的 `4×3` 漫画插画板，程序再按顺序裁切。
4. 程序在本地排版成单张 `1440×2400` PNG，完成后提供可滚动预览。
...
| 生成图片日报 | 挑选最多 10 个重点并输出单张彩色漫画信息图 PNG |
