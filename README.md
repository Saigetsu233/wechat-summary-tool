...
3. 启用“AI 话题插画”时，标准模式会用 `gemini-3.1-flash-image` 一次生成无文字的 `4×3` 手绘漫画素材板，再按固定栏目裁切；需要更贴合每条议题时，勾选“精致手绘模式”，程序会为 6 个话题和最多 3 位真实上榜群友逐张绘制。
4. 程序在本地排版成单张 `1536×1536` PNG；栏目位置和整体风格不会随模型输出改变。

图片中的中文仍由程序本地排版，因此不会出现 AI 图片中常见的中文乱码；图片模型只绘制无文字的漫画插画。默认图片模型为 `gemini-3.1-flash-image`；追求更强的手绘细节可在“图片模型”选择 `gemini-3-pro-image`，但费用更高。配图使用 Gemini API Key：如果文字总结也选择 Gemini，就会直接共用当前 Key；如果文字选择 DeepSeek/NVIDIA，则需要先在 Gemini 服务项中保存一次 Key。关闭“AI 话题插画”后不会调用图片接口。如果刚刚已生成同一群、同一日期的文字总结，图片功能会直接复用，避免重复总结。
